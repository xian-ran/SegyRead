...
def Wiggle(data, lWidth):
    ...
    for i in range(traceNum):
        ...
        plt.plot(i+traceData/maxVal, [j for j in range(sampleNum)], color='black', linewidth=lWidth)
        for a in range(len(traceData)):
            if(traceData[a] < 0):
                traceData[a] = 0
        plt.fill(i+traceData/maxVal, [j for j in range(sampleNum)], 'k', linewidth=0)
    #pylab.grid(True)
    plt.axis([0,traceNum,sampleNum,0])
    plt.show()
